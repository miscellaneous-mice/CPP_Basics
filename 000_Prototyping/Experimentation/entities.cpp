#include "entities.h"

template bool HashMap::add_element(const std::string&, A*);
template bool HashMap::add_element(const std::string&, B*);
template bool HashMap::add_element(const std::string&, Base*);


template A* HashMap::get_element<A>(const std::string&);
template B* HashMap::get_element<B>(const std::string&);
template Base* HashMap::get_element<Base>(const std::string&);

template<typename T>
bool HashMap::add_element(const std::string& key, T value) {
    if (handlers.find(key) != handlers.end()) {
        return false;
    }
    this->handlers[key] = value;  // Store value as std::any
    return true;
}

template<typename T>
T* HashMap::get_element(const std::string& key) {
    if (handlers.find(key) == handlers.end()) {
        std::cerr<<"The handler does not exist"<<std::endl;
        return nullptr;
    }
    try {
        // return dynamic_cast<T*>(std::any_cast<Base*>(this->handlers[key])); 
        return std::any_cast<T*>(this->handlers[key]);
    } catch (const std::bad_any_cast& e) {
        // std::cerr << "Bad cast: " << e.what() << std::endl;
        return nullptr;
    }
}

bool HashMap::remove_element(const std::string& key){
    if (handlers.find(key) != handlers.end()) {
        this->handlers.erase(key);
        return true;
    }
    return false;
}

HashMap::~HashMap(){
    for (const auto& pair : this->handlers) {
        if (auto entity = get_element<A>(pair.first)) {
            delete entity;
        }
        else if (auto entity = get_element<B>(pair.first)) {
            delete entity;
        }
        else if (auto entity = get_element<Base>(pair.first)) {
            delete entity;
        }
    }
}

void Base::info(){
    std::cout<<"Base Class"<<std::endl;
}

void A::info() {
    std::cout<<"A Class"<<std::endl;
}

void B::info() {
    std::cout<<"B Class"<<std::endl;
}