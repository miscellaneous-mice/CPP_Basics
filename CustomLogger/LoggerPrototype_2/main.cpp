...
int main(){
    ...
    BaseUtils* consoleutils = logger->StreamHandler(Configs::LogLevelError, "%d-%m-%Y %H-%M-%S", "date::level::message");
    BaseUtils* fileutils = logger->FileHandler(Configs::LogLevelWarning, "%d:%m:%Y %H:%M:%S", "date::level::message");
