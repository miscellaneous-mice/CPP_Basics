...
template bool Handlers::add_element(const std::string&, FileUtils*);
template bool Handlers::add_element(const std::string&, ConsoleUtils*);
template bool Handlers::add_element(const std::string&, BaseUtils*);


template FileUtils* Handlers::get_element<FileUtils>(const std::string&);
template ConsoleUtils* Handlers::get_element<ConsoleUtils>(const std::string&);
template BaseUtils* Handlers::get_element<BaseUtils>(const std::string&);

template<typename T>
bool Handlers::add_element(const std::string& key, T value) {
...
template<typename T>
T* Handlers::get_element(const std::string& key) {
    if (handlers.find(key) == handlers.end()) {
...
Handlers::~Handlers(){
    for (const auto& pair : this->handlers) {
        if (auto entity = get_element<ConsoleUtils>(pair.first)) {
            delete entity;
        }
        else if (auto entity = get_element<FileUtils>(pair.first)) {
            delete entity;
        }
        else if (auto entity = get_element<BaseUtils>(pair.first)) {
            delete entity;
        }
    }
}
